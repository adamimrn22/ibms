...
trait LaptopFilterTraits
{
    public function applyPaginationFilterSearch($query, $perPage, $searchTerm,  $status)
    {
        ...
        // Apply pagination
        $results = $query->select('name', DB::raw("JSON_UNQUOTE(JSON_EXTRACT(attribute, '$.model')) AS model, JSON_UNQUOTE(JSON_EXTRACT(attribute, '$.brand')) AS brand"), 'location', 'status', 'id')->paginate($perPage);
