...
$(document).ready(function () {
    ...
    $(document).on('click', '#Pagination a', function (e) {
        ...
        let searchTerm = $('#searchProjector').val();
    ...
    $(document).on('keyup', '#searchProjector', function () {
    ...
    $(document).on('change', '#recordFilter', function () {
        ...
        let searchTerm = $('#searchPrinter').val();
        let status = $('#statusFilter').val();
        fetch_data(1, searchTerm, status, recordsPerPage);
    });
    ...
    function fetch_data(page, searchTerm = '', status = '', recordsPerPage = '') {
        $.ajax({
            ...ajaxSettings,
            url: `${baseUrl}/Inventory/UIT/Hardware/Projector`,
            type: "GET",
            data: {
                page: page,
                search: searchTerm,
            ...
            success: function (data) {
                spinnerContainer.hide();
                $('#projectorTable').html(data.table).show();
