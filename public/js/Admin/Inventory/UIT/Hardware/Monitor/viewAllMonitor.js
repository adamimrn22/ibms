...
$(document).ready(function () {
    ...
    $(document).on('click', '#Pagination a', function (e) {
        ...
        let searchTerm = $('#searchMonitor').val();
    ...
    $(document).on('keyup', '#searchMonitor', function () {
    ...
    $(document).on('change', '#recordFilter', function () {
        ...
        let searchTerm = $('#searchLaptop').val();
        let status = $('#statusFilter').val();
        fetch_data(1, searchTerm, status, recordsPerPage);
    });
    ...
    function fetch_data(page, searchTerm = '', status = '', recordsPerPage = '') {
        $.ajax({
            ...ajaxSettings,
            url: `${baseUrl}/UIT/Inventory/Hardware/Monitor`,
            type: "GET",
            data: {
                page: page,
                search: searchTerm,
            ...
            success: function (data) {
                spinnerContainer.hide();
                $('#monitorTable').html(data.table).show();
