...
@section('layout')
    <x-app-content>
        ...
        {{ Breadcrumbs::render('car.edit', $car) }}
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @endif
                        <form id="updateCarForm" action="{{ route('upsm.Kenderaan.update', ['Kenderaan' => $car->id]) }}"
                            method="POST">
                            @method('PUT')
                            @csrf
                            <div class="row">
                                <x-form.form-group>
                                    ...
                                    <x-form.input :id="'name'" :value="$car->name" />
                                </x-form.form-group>

                                <x-form.form-group>
                                    <x-form.label :for="'location'" :title="'Location'" />
                                    <x-form.input :id="'location'" :value="$car->location" />
                                </x-form.form-group>
                                ...
                                <x-form.form-group>
                                    ...
                                    <input type="text" id="DOP" name="DOP" value="{{ $car->attribute->DOP }}"
                                        ...
                                        <select style="overflow:hidden" id="status" name="status"
                                            ...
                                            @foreach ($statuses as $status)
                                                <option value="{{ $status->id }}"
                                                    {{ $car->status_id == $status->id ? 'selected' : '' }}>
                                                    {{ $status->name }}
                                                </option>
                                            @endforeach
                                        </select>
...
@section('script')
    ...
    <script src="{{ asset('js/Admin/Inventory/UPSM/Car/editCar.js') }}"></script>
@endsection
