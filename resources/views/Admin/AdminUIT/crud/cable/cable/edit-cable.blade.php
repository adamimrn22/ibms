...
@section('layout')
    <x-app-content>
        ...
        {{ Breadcrumbs::render('cable.edit', $cable) }}

        <x-uit.card title="Add New Cable">
            <x-form :id="'createCableForm'" :action="route('uit.Cable.update', ['Cable' => $cable->id])" :method="'POST'">

                <x-form.form-group>
                    <x-form.label :for="'cableID'" :title="'Cable ID'" />
                    <x-form.input :id="'cableID'" :value="$cable->name" />
                </x-form.form-group>

                <x-form.form-group>
                    <x-form.label :for="'price'" :title="'Price (RM)'" />
                    <x-form.input :id="'price'" :value="$cable->price" :type="'number'" />
                </x-form.form-group>

                <x-form.form-group>
                    <x-form.label :for="'cableName'" :title="'Cable Name'" />
                    <x-form.input :id="'cableName'" :value="$cable->attribute->cableName" />
                </x-form.form-group>

                <x-form.form-group>
                    <x-form.label :for="'location'" :title="'Location'" />
                    <x-form.input :id="'location'" :value="$cable->location" />
                </x-form.form-group>
                        ...
                        <select style="overflow:hidden" id="subcategory_id" name="subcategory_id"
                            ...
                            @foreach ($subcategories as $subcategory)
                                <option value="{{ $subcategory->id . '|' . $subcategory->subcategory_name }}"
                                    {{ $cable->subcategory_id == $subcategory->id ? 'selected' : '' }}>
                ...
                <x-form.form-group>
                    ...
                    <x-form.input :id="'meter'" :value="$cable->attribute->meter" type="number" />
                </x-form.form-group>

                <x-form.form-group>
                    <x-form.label :for="'DOP'" :title="'Date Of Purchase'" />
                    <input type="text" id="DOP" name="DOP" value="{{ $cable->attribute->DOP }}"
                        ...
                        <select style="overflow:hidden" id="status" name="status"
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status->id }}"
                                    {{ $cable->status_id == $status->id ? 'selected' : '' }}>
                                    {{ $status->name }}
                                </option>
                            @endforeach
                        </select>
